...
LOGO_ID = "rbxassetid://74152855887666"
task.spawn(function() pcall(function() ContentProvider:PreloadAsync({LOGO_ID}) end) end)

_isfile = isfile or (syn and syn.isfile) or (getgenv and getgenv().isfile) or function() return false end
_readfile = readfile or (syn and syn.readfile) or (getgenv and getgenv().readfile) or function() return nil end
_writefile = writefile or (syn and syn.writefile) or (getgenv and getgenv().writefile) or function() end
...
autoTpDownYTarget = -8.80
autoTpDownThreshold = 6
autoTpDownJumpBoost = 75
autoTpDownFallMultiplier = 3.5
lastAutoTpTime = 0
AUTO_TP_COOLDOWN = 0.2
...
function shouldSteal(animalData)
...
-- ============================================================
function updateStealUI(progress)
...
function startSmoothProgress(duration, animalData)
...
function executeInternalStealAsync(prompt, animalData, duration)
...
function attemptSteal(prompt, animalData, duration)
...
-- ============================================================
Presets = {}
PRESET_FILE = "CleanHubPresets.json"
LAST_PRESET_FILE = "CleanHubLastPreset.json"
CONFIG_FILE = "CleanHubConfig.json"
POSITIONS_FILE = "CleanHubPositions.json"
BATV2_POS_FILE = "CleanHubBatV2Pos.json"

function buildPresetSnapshot() return {} end
function savePresetsFile() end
function loadPresetsFile() end
function saveLastPresetName(name) end
function loadLastPresetName() return nil end
function rebuildPresetList() end
...
h, hrp = nil, nil
setAutoLeft, setAutoRight, setInfJump, setAntiRag = nil, nil, nil, nil
setMedusaCounter, setUnwalkToggle, setAimbot = nil, nil, nil
setLagger, setDropBrainrot, setInstaGrab, setAutoTpDown = nil, nil, nil, nil
setupMedusaCounter, stopMedusaCounter, startAntiRagdoll, stopAntiRagdoll = nil, nil, nil, nil
runDropBrainrot, stopDropBrainrot, doTpDown = nil, nil, nil
startBatAimbot, stopBatAimbot, startBatCounter, stopBatCounter, setBatCounter = nil, nil, nil, nil, nil
startAutoTpDown, stopAutoTpDown = nil, nil
stackBtnRefs = {}; local keybindBtnRefs = {}
normalBox, carryBox, laggerBox, lagguerBox, stealRadBox, thresholdBox = nil, nil, nil, nil, nil, nil
jumpModeContainer, manuelBtn, holdBtn = nil, nil, nil
setStunTimerToggle, setLockUIToggle, setAutoStealToggle, setBatV2Toggle, setInfJumpToggle, setAntiRagdollToggle, setMedusaCounterToggle, setBatCounterToggle, setAutoTpDownToggle = nil, nil, nil, nil, nil, nil, nil, nil, nil

-- ============================================================
-- COLORES ROSA / MORADO (tema claro-oscuro mezclado)
-- ============================================================
WHITE_PURE = Color3.fromRGB(255,255,255)
BLACK_PURE = Color3.fromRGB(0,0,0)

PURPLE_ACCENT = Color3.fromRGB(170,85,255)
PINK_ACCENT = Color3.fromRGB(170,85,255)
DARK_PURPLE = Color3.fromRGB(25,10,45)
DARK_PINK_BG = Color3.fromRGB(10,10,10)
ROW_BG = Color3.fromRGB(22,22,22)
TOGGLE_BAR_BG = Color3.fromRGB(0,0,0)
MOBILE_BTN_BG = Color3.fromRGB(0,0,0)
MOBILE_BTN_ACTIVE = Color3.fromRGB(170,85,255)
LIGHT_PINK = Color3.fromRGB(210,190,255)
PINK_INPUT_BG = Color3.fromRGB(0,0,0)

BLACK_BG = Color3.fromRGB(10,10,10)
LIGHT_PURPLE = Color3.fromRGB(210,190,255)
...
uiScaleObj = Instance.new("UIScale", gui)
uiScaleObj.Scale = 1.0

-- ============================================================
-- FUNCIONES UI
-- ============================================================
function mkCorner(p, r) local c = Instance.new("UICorner", p); c.CornerRadius = UDim.new(0, r or 6); return c end
function mkStroke(p, col, th) local s = Instance.new("UIStroke", p); s.Color = col; s.Thickness = th or 1; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; return s end
...
function repositionBatV2ToLeftOfAimbot()
    task.wait(0.05)
...
function makeDraggable(frame, onPositionChanged)
...
-- ============================================================
toggleBar = Instance.new("Frame", gui)
...
label = Instance.new("TextLabel", toggleBar)
...
clickButton = Instance.new("TextButton", toggleBar)
...
bgOverlay = Instance.new("Frame", mainOuter)
...
-- ========== BARRA SUPERIOR ==========
titleBar = Instance.new("Frame", mainOuter)
...
titleDiv = Instance.new("Frame", mainOuter)
...
-- ============================================================
CONTENT_Y = TITLE_H + 1
contentScroller = Instance.new("ScrollingFrame", mainOuter)
...
mainColumn = Instance.new("Frame", contentScroller)
...
function makeInputRow(label, default, onChange)
...
function makeToggleRow(label, defaultOn, onToggle)
    local row = Instance.new("Frame")
    row.Size = UDim2.new(1, 0, 0, 34)
    row.BackgroundTransparency = 1
    row.BorderSizePixel = 0
...
function makeKeybindRow(label, currentKey, onChanged, keyName)
    local row = Instance.new("Frame")
    row.Size = UDim2.new(1, 0, 0, 34)
    row.BackgroundTransparency = 1
    row.BorderSizePixel = 0
...
function createStunTimerBillboard()
...
function setupStunDetection(char)
...
setBatV2Toggle = makeToggleRow("Bat V2", false, function(on)
...
grid = Instance.new("UIGridLayout", buttonContainer)
...
batV2Frame = Instance.new("Frame", batV2Container)
...
strokeV2 = mkStroke(batV2Frame, PURPLE_ACCENT, 1)
strokeV2.Transparency = 0.8

batV2Lbl = Instance.new("TextLabel", batV2Frame)
...
batV2Click = Instance.new("TextButton", batV2Frame)
...
positionLoaded = loadBatV2Position()
...
BAT_V2_AIMBOT_SPEED = 60
BAT_V2_SWING_COOLDOWN = 0.08
BAT_V2_HIT_DIST = 4.5
...
startBatV2Aimbot = function()
    ...
    local hum0 = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum0 then
        if _prevBatV2AutoRotate == nil then _prevBatV2AutoRotate = hum0.AutoRotate end
        hum0.AutoRotate = false
    end
    ...
    Conns.batV2Aimbot = RunService.Heartbeat:Connect(function()
        ...
        -- Se desligar, para imediatamente de olhar/inclinar
        if not State.batV2Toggled then
            hum.AutoRotate = true
            root.AssemblyAngularVelocity = Vector3.zero
            return
        end
        ...
        if target and target.Character then
            ...
            if targetRoot then
                ...
                -- INCLINAÃ‡ÃƒO + OLHAR DO AIMBOT NORMAL NO AIMBOT V2
                local myPos = root.Position
                local targetPos = targetRoot.Position
                local speed3 = targetVel.Magnitude
                local predictTime = math.clamp(speed3 / 150, 0.05, 0.2)
                local predictedPos = targetPos + targetVel * predictTime
                predictedPos = predictedPos + targetRoot.CFrame.LookVector * 0.3
...
stopBatV2Aimbot = function()
    if Conns.batV2Aimbot then
        Conns.batV2Aimbot:Disconnect()
        Conns.batV2Aimbot = nil
    end
    ...
    if hum then
        hum.AutoRotate = (_prevBatV2AutoRotate == nil) and true or _prevBatV2AutoRotate
        hum.PlatformStand = false
        pcall(function()
...
UIS.InputBegan:Connect(function(inp, gp)
    if gp then return end
    if inp.KeyCode == Keys.batV2 then
        State.batV2Toggled = not State.batV2Toggled
        if State.batV2Toggled then
            startBatV2Aimbot()
        else
        ...
        if setBatV2Toggle then pcall(setBatV2Toggle, State.batV2Toggled) end
        if saveConfig then pcall(saveConfig) end
    end
end)
